...
const ProductTabsComment = ({ productName = ' Продукт' }) => {
    ...
    const sendComment = (data) => {
        updateDoc(doc(db, "products", product.id), {
            ...
            comments: [
                ...product?.comments, {
                    ...data, stars, id: product.comments[product.comments.length - 1]?.id ?
                ...
                <div className={'product__tabs-comment-left'}>
                    {
                        overlay === true ?
                            <>
                                <div className={'formalize__overlay'} onClick={() =>  setOverlay(false) }> </div>
                                <div className="formalize__popup">
                                    <div className={'formalize__popup-close'} onClick={() => setOverlay(false)}>
...

                    {
                        product?.comments.length ?
                            product?.comments?.map((item, idx) => (
                                <div key={idx} className={'product__tabs-comment shadow-box'}>
                                    <div className={'product__tabs-comment-top'}>
                                        <img src={face} alt="face" className={'product__tabs-comment-img'} />
                                        <span>{item.name}</span>
                                        <span className={'product__tabs-comment-rate'}>
