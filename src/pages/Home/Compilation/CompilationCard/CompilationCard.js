...
const CompilationCard = ({title, img, price, city, sell, id, creator, countInRow}) => {
    ...
    const addFav = (obj) =>{
        ...
        //     .then(async (res) => {
        //        await dispatch(findUser({obj:obj}));
        //        await updateDoc(doc(db, 'user', res.docs.map(el => ({...el.data(), id:el.id}) ).find(item => item.email === user.email).id ),obj)
        //     });
